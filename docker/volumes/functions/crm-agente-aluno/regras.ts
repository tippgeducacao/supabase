 ...
 * único que não acionava nada, porque a conclusão dependia de o modelo chamar uma ferramenta e
 * o prompt pedia que ele CALASSE diante do botão.
 *
...
/**
 * Os títulos dos botões dos modelos VIVOS, normalizados.
 *
...
export function montarContexto(c: ContextoAluno, o: OpcoesContexto): string {
  ...
  if (c.passo_fechou_quando) {
    linhas.push(
      `- O que conta como o passo de hoje CONCLUÍDO: ${s(c.passo_fechou_quando)}. ` +
      'Se foi isso que aconteceu, use marcar_passo_concluido.',
    );
  }
