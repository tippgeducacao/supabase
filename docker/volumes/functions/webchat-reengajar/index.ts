...
import { enviarPush, getWebpush, webpushErro } from "../_shared/webchatPush.ts";
...
// resultado por tentativa (endpoint truncado + status) — vai no response (push_tentativas)
const pushTentativas: string[] = [];
...
Deno.serve(async (req) => {
  ...
  for (const s of (sessoes ?? []) as any[]) {
    ...
    if (wp) {
      ...
      for (const sub of (subs ?? []) as any[]) {
        const { r, detalhe } = await enviarPush(wp, sub, payload);
        pushTentativas.push(detalhe);
  ...
  return json({ ok: true, total: (sessoes ?? []).length, push: porPush, whatsapp: porWhatsapp, sem_sucesso: semSucesso, push_debug: webpushErro(), push_tentativas: pushTentativas.slice(-10) });
});
