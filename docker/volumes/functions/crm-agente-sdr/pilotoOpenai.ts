...
export async function selecionarProvedorDoLead(supabase: any, telefone: string): Promise<ProvedorIA | null> {
  try {
    const { data, error } = await supabase.from('crm_agente_sdr_config').select('luna_telefones, openai_modelo_piloto').eq('id', 1).maybeSingle();
    ...
    const provedor = provedorOpenai();
    return provedor?.formato === 'openai' && modeloOpenaiPermitido(data.openai_modelo_piloto)
      ? { ...provedor, modelo: data.openai_modelo_piloto }
      : provedor;
  } catch { return null; }
}
